...
interface AskQuestionViewProps {
  currentUser: User;
  onPostQuestion: (title: string, body: string, tags: string[]) => void;
  onCancel: () => void;
}
...
export default function AskQuestionView({
  ...
  onPostQuestion,
  onCancel,
}: AskQuestionViewProps) {
  const [title, setTitle] = useState('');
  const [body, setBody] = useState('');
  const [tagInput, setTagInput] = useState('');
  const [error, setError] = useState('');

  // Generate real-time tag chips as user types
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onPostQuestion(title.trim(), body.trim(), tags);
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="bg-white dark:bg-zinc-950 p-5 rounded-xl border border-zinc-200 dark:border-zinc-800 space-y-3 shadow-xs">
              ...
              {/* Real-time Tag Preview Chips render */}
              {tagChips.length > 0 && (
        ...
        {/* Right sidebar column: Helpful hints & guidelines */}

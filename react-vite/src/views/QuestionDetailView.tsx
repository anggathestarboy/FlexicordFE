...
interface QuestionDetailViewProps {
  ...
  currentUser: User;
  onBackClick: () => void;
  onQuestionVote: (id: string, dir: 'up' | 'down') => void;
  onAnswerVote: (questionId: string, answerId: string, dir: 'up' | 'down') => void;
  onAddAnswer: (questionId: string, body: string) => void;
  onAddCommentToQuestion: (questionId: string, body: string) => void;
  onAddCommentToAnswer: (questionId: string, answerId: string, body: string) => void;
...
export default function QuestionDetailView({
  ...
  const handleAddAnswerSubmit = (e: React.FormEvent) => {
    ...
    onAddAnswer(question.id, newAnswerBody);
    setNewAnswerBody('');
  };
  ...
  const handleAddQuestionComment = (e: React.FormEvent) => {
    ...
    onAddCommentToQuestion(question.id, questionComment);
    setQuestionComment('');
  ...
  const handleAddAnswerComment = (ansId: string, e: React.FormEvent) => {
    ...
    onAddCommentToAnswer(question.id, ansId, commentText);
    setAnswerCommentStates(prev => ({ ...prev, [ansId]: '' }));
    setShowAnswerCommentInputs(prev => ({ ...prev, [ansId]: false }));
  };

  // Help format blocks of markdown code securely without extra external parser dependencies
  ...
  return (
    <div className="space-y-6">
      {/* Back button and simple navigation */}
      <button
        onClick={onBackClick}
      ...
      {/* CORE GRID: Votes column + Question body column */}
      <div className="flex gap-4 sm:gap-6">
        {/* Left column: Voting controls */}
        <div className="flex flex-col items-center gap-1.5 text-center shrink-0">
          <button
            onClick={() => onQuestionVote(question.id, 'up')}
          ...
          <button
            onClick={() => onQuestionVote(question.id, 'down')}
            className={`p-1.5 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-950 transition-colors cursor-pointer ${
              question.voted === 'down'
        ...
        {/* Right column: Question content and attachments */}
        <div className="flex-1 min-w-0 pr-1 sm:pr-2">
          ...
          {/* Question bottom Author signature block */}
          ...
          <div className="mt-8 pt-4 border-t border-zinc-100 dark:border-zinc-900/60 space-y-3.5">
            ...
            {/* Comment Form toggled with a custom action link */}
              ...
              >
                <div className="flex gap-4 sm:gap-6">
                  ...
                  <div className="flex flex-col items-center gap-1 text-center shrink-0 pt-1">
                    ...
                    <button
                      onClick={() => onAnswerVote(question.id, ans.id, 'down')}
                      className={`p-1 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-900 cursor-pointer ${
                        ans.voted === 'down'
                  ...
                  <div className="flex-1 min-w-0 pr-1">
                    {/* Render Formatted text details */}
                    <article className="prose prose-zinc dark:prose-invert max-w-none text-zinc-800 dark:text-zinc-200">
                      {formatTextBody(ans.body)}
                    </article>

                    {/* Answer bottom metadata */}
      ...
      <div className="mt-8 pt-6 border-t border-zinc-200 dark:border-zinc-850 space-y-4">
        ...
        {/* Markdown mini guidelines helper */}

...
interface HomeViewProps {
  ...
  onVoteUp: (id: string, e: React.MouseEvent) => void;
  onVoteDown: (id: string, e: React.MouseEvent) => void;
  ...
  const filteredQuestions = questions
    ...
    .sort((a, b) => {
      ...
      if (activeSort === 'belum-terjawab') {
        // Unanswered first, then those with fewer answers
        return a.answers.length - b.answers.length;
      }
      // 'terbaru' (default, simulated keeping newer items first or q-id order)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-zinc-200 dark:border-zinc-800">
        ...
        {/* Large action button solely for responsive views */}
        <button
        ...
        <div className="space-y-4">
          {filteredQuestions.map((q) => (
            <QuestionCard
              ...
              onVoteUp={(e) => onVoteUp(q.id, e)}
              onVoteDown={(e) => onVoteDown(q.id, e)}
            />
          ))}
        </div>
      ) : (

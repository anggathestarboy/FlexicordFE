...
function FollowingCard({
  ...
  const src = avatarSrc(following.avatar_url);
  const lvColor = levelColor(following.level);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {following.bio ? (
          <p className="text-xs text-zinc-400 truncate mt-0.5">{following.bio}</p>
        ) : (
        ...
        <div className="flex items-center gap-1.5 mt-1.5">
          ...
          <span className="text-[10px] font-mono font-bold" style={{ color: lvColor }}>
            {following.reputation_points.toLocaleString()} pts
          </span>
        </div>
      </div>
...
export default function FollowingPage() {
  ...
  const fetchFollowings = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/followings/${encodeURIComponent(username)}`);
      if (res.status === 404) {
        setFollowings([]);
        setFiltered([]);
        return;
      }
      if (!res.ok) throw new Error("Gagal memuat daftar mengikuti.");
      const data = await res.json();
      const parsed: FollowingUser[] =
        typeof data.following === "string"
          ? JSON.parse(data.following)
          : Array.isArray(data.following)
          ? data.following
          : [];
      setFollowings(parsed);
      setFiltered(parsed);
    } catch (e: unknown) {
    ...
    <div className="space-y-5">
      ...
      {!loading && !error && followings.length > 0 && (
        <div className="relative">
          ...
          <input
            id="input-search-following"
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Cari pengguna berdasarkan username atau bio…"
      ...
      <div className="space-y-2">
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-1">
                @{username} belum mengikuti pengguna lain.
              </p>
            </div>
          </div>
        )}
        ...
        {!loading && !error && followings.length > 0 && filtered.length === 0 && (
      ...
      {!loading && !error && filtered.length > 0 && (
        <p className="text-center text-[11px] text-zinc-400 font-mono pb-2">
          Menampilkan {filtered.length} dari {followings.length} yang diikuti
        </p>
      )}
    </div>
  );

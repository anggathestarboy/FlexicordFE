  ...
  try {
    ...
    if (!reason || typeof reason !== "string") {
      return NextResponse.json(
        {
          success: false,
          message: "Alasan ban (reason) wajib diisi.",
        },
        { status: 400 }
      );
    }
    ...
    if (reason.length > 255) {
      return NextResponse.json(
        {
          success: false,
          message: "Alasan ban (reason) tidak boleh melebihi 255 karakter.",
        },
        { status: 400 }
      );
    }
    ...
    const response = await fetch(`${BASE_URL}/banned/${encodeURIComponent(username)}`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          success: false,
          message: result.message || "Gagal membanned user.",
    ...
    return NextResponse.json(
      {
        success: true,
        message: result.message || "User berhasil dibanned.",
        data: result.data,
      },
      { status: response.status }
    );
